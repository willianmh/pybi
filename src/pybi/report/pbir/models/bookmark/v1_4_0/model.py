...
from ...semanticquery.expressions import (
    FilterDefinition,
...
class VisualContainerGroupState(BaseModel):
    ...
    children: dict[str, VisualContainerGroupState] | None = Field(
...
class Bookmark(BaseModel):
    ...
    options: BookmarkOptions | None = Field(
        None, description="Additional options for the bookmark"
    )
...
class ExplorationState(BaseModel):
    ...
    filters: FiltersState | None = Field(
        None,
    ...
    objects: DataViewObjectDefinitionUpdates | None = Field(
        None, description="Changes to formatting to apply in this bookmark."
    )
...
class FilterContainerState(BaseModel):
    ...
    filter: FilterDefinition | None = Field(
        None, description="Defines a filter element as a partial query structure"
    )
    ...
    filterExpressionMetadata: (
        FilterExpressionMetadata | DecomposedFilterExpressionMetadata | None
    ) = None
...
class FilterExpressionMetadata(BaseModel):
    ...
    cachedValueItems: list[IdentityValueMap] | None = Field(
        None,
...
class DecomposedFilterExpressionMetadata(BaseModel):
    ...
    decomposedIdentities: DecomposedIdentities | None = Field(
        None,
    ...
    valueMap: (
        list[dict[Annotated[str, StringConstraints(pattern=r"^[0-9]+$")], str]] | None
    ) = Field(
        None,
    ...
    values: list[
        list[
            dict[
                ...
                list[QueryExpressionContainer | QueryExpressionContainer],
            ]
        ]
    ] = Field(
        ...,
...
class DecomposedTreeQueryExpressionContainer(BaseModel):
    ...
    value: QueryExpressionContainer | None = Field(
        None,
        description="Holds a single expression and associated metadata.\nName, NativeReferenceName, and Annotations may be specified for any expression.\nEach other property represents a specific type of expression and exactly one of these other properties must be specified.",
    )
...
class SectionState(BaseModel):
    ...
    visualContainerGroups: dict[str, VisualContainerGroupState] | None = Field(
        None,
...
class VisualContainerState(BaseModel):
    ...
    singleVisual: SingleVisualConfigState | None = Field(
        None, description="State of the configuration of the visual."
    )
    highlight: HighlightState | None = Field(
        None, description="Cross-highlights captured in the bookmark."
    )
...
class SingleVisualConfigState(BaseModel):
    ...
    activeProjections: (
        ...
        | None
    ) = Field(None, description="Updated projections that are used by the visual.")
    projections: (
        dict[
            str,
            list[QueryExpressionContainer | QueryExpressionContainer],
    ...
    parameters: dict[str, list[ParameterState]] | None = Field(
        None,
    ...
    display: VisualContainerDisplayState | None = Field(
        None, description="Optional changes to how the visual is displayed."
    )
...
class DataViewObjectDefinitionUpdates(BaseModel):
    ...
    merge: dict[str, list[DataViewObjectDefinition]] | None = Field(
        None, description="ObjectDefinitions to be merged with the target."
    )
    remove: list[DataViewObjectPropertyIdWithSelector] | None = Field(
...
class DecomposedSelectors(BaseModel):
    ...
    queryNameMap: (
        list[dict[Annotated[str, StringConstraints(pattern=r"^[0-9]+$")], list[float]]]
        | None
    ) = Field(
        None,
